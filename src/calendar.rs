    ...
    Tz::Offset: Display,
{
    ...
    fn show_grid_header(&mut self, ui: &mut Ui) {
        ...
        for i in day_indexes {
            let b = Weekday::try_from(i).unwrap();
    ...
    fn show_month_control(&mut self, ui: &mut Ui) {
        ...
        // let month_string = chrono::Month::try_from(self.date.month()).unwrap().name();
        // let month_string = u8::try_from(self.date.month()).name();
        let month_string = Month::try_from(u8::try_from(self.date.month()).unwrap()).unwrap().name();
...
fn get_days_from_month(year: i32, month: u32) -> i64 {
    ...
    .signed_duration_since(NaiveDate::from_ymd_opt(year, month, 1).expect("msg"))
    .num_days()
}
